...
def get_dd_wind_field(Grids, u_init, v_init, w_init, points=None, vel_name=None,
    ...
    for param_name in dir(params):
        if '__' not in param_name:
            ...
            if isinstance(curr_param, (float, np.ndarray, np.float128, np.float64, np.float32)):
                if prec is 'half':
                    mydict[param_name] = np.float16(curr_param)
                if prec is 'single':
                    mydict[param_name] = np.float32(curr_param)
                if prec is 'double':
                    mydict[param_name] = np.float64(curr_param)
                ...
                if len(curr_param) > 0:
                    for ii, entry in enumerate(curr_param):
                        if isinstance(entry, (np.ndarray, np.float128, np.float64, np.float32)):
                            if prec is 'half':
                                mydict[param_name][ii] = np.array(entry, dtype='float16')
                            if prec is 'single':
                                mydict[param_name][ii] = np.array(entry, dtype='float32')
                            if prec is 'double':
                                mydict[param_name][ii] = np.array(entry, dtype='float64')
    ...
    for i in range(n_runs):
        x = np.random.normal(0,1, (3, params.grid_shape[0], params.grid_shape[1], params.grid_shape[2]) ) #winds.shape)
        x = np.array(x, dtype=data_type)
        ti = time.time()
        ret = fun(x, 'single')
        tf = time.time()
        print('Run', i+1, 'took', tf-ti,'seconds')
        avg_time = (i*avg_time + (tf-ti)) / (i + 1)
    print("Total average time is", avg_time)
    t0 = time.time()
    temp = list()
