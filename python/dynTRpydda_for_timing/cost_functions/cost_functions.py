...
def get_correct_one(x):
    if isinstance(x, np.float16):
        correct_one = np.float16(1.0)
...
def change_precision(winds, params, precision):
    ...
    winds = np.reshape(winds, (3, params.grid_shape[0], params.grid_shape[1], params.grid_shape[2]))
...
def J_function(winds, parameters, precision):
    ...
    # pass iterate and parameters to function that changes precision
    if winds.dtype != parameters.weights.dtype:
        print("Precisions don't match...try again")

    #winds, parameters = change_precision(winds, parameters)
...
def grad_J(winds, parameters, precision):
    ...
    """
    #winds = np.reshape(winds, (3, parameters.grid_shape[0],
