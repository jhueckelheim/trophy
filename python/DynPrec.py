...
def DynTR(x0, handle, prec_vec, epsilon, budget):
    ...
    for i in range(budget):   # note that this indexes from 0 and not 1, check for errors because of it
        ...
        if first_success == 0:
            sol = CG_Steinhaug_matFree(epsTR, g , delta, S, Y, x0.shape[0])
        ...
        s = sol #sol[1]          # we will likely need to change the indices here for a different solver
        #s = s[:,1]
        val = 0.5*(s@H)@s + s@g
...
# page Ch7 pp. 183 nocedal and wright
