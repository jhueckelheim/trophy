        ...
        if prob_str not in ['DEVGLA2NE', 'JIMACK', 'S308NE'] and 'BA-' not in prob_str:
            if os.path.exists(cache_dir+prob_str+"_single"):
                ...
                if p1.n <= max_problem_dim:
                    ...
                    ret = DynPrec.DynTR(x0, func_single, {'single': 1}, gtol=eps, max_iter=maxit, tr_tol=epsTR, verbose=False, max_memory=max_memory, norm_to_use=2)
                    ...
                    if ret.success:
                        ...
                        success = 'failed'
                    temp = [prob_str, success, t_elapsed, ret.fun, norm(ret.jac), ret.nit, ret.nfev, ret.precision_counts['single'], ret.message]
                    ...
                    ret = DynPrec.DynTR(x0, func_double, {'double': 2}, gtol=eps, max_iter=maxit, tr_tol=epsTR, verbose=False, max_memory=max_memory, norm_to_use=2)
                    t_elapsed = time.time() - t0
                    mystr = '(' + ret.message[0:25] + ')'
                    print(mystr)
                    if ret.success:
                    ...
                    ret = DynPrec.DynTR(x0, func_dynamic, {'single': 1, 'double': 2}, gtol=eps, max_iter=maxit, tr_tol=epsTR, verbose=False, max_memory=max_memory, norm_to_use=2)
                    t_elapsed = time.time() - t0
                    mystr = '(' + ret.message[0:25] + ')'
                    print(mystr)
                    if ret.success:
