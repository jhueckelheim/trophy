...
cache_dir = '/Users/clancy/Documents/_research/vp_trust/pycutest_cache/pycutest_cache_holder/'
        ...
        if 'JIMACK' not in prob_str and 'BA-' not in prob_str:
            if os.path.exists(cache_dir+prob_str+"_single"):
                ...
                if p1.n <= max_problem_dim:
                    ...
                    ret = scipy.optimize.minimize(func_bfgs, x0, method="L-BFGS-B", jac=True, bounds=None, options={'ftol': 0, 'gtol': eps, 'maxcor': max_memory, 'maxiter': budget})
                    t_elapsed = time.time() - t0
